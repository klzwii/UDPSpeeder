...
namespace galois {
    gf logTable[256] = {0, 255, 1, 25, 2, 50, 26, 198, 3, 223, 51, 238, 27, 104, 199, 75, 4, 100, 224, 14, 52,
    ...
    gf antiLogTable[256] = {1, 2, 4, 8, 16, 32, 64, 128, 29, 58, 116, 232, 205, 135, 19, 38, 76, 152, 45, 90, 180, 117,
    ...
    gf mulTables[256][256];
    gf divTables[256][256];
...
void galois::initGF() {
    ...
    while (!inited.compare_exchange_strong(unInitialize, true)) {
        if (inited.load()) {
            return;
        }
    }
    for (int i = 1; i < 256; i++) {
        for (int j = 1; j < 256; j++) {
            int k = (logTable[i] + logTable[j]) % 255;
            mulTables[i][j] = antiLogTable[k];
        }
    }
    for (int i = 0; i < 256; i++) {
        mulTables[0][i] = 0;
    ...
    for (int i = 0; i < 256; i++) {
        for (int j = 1; j < 256; j++) {
            divTables[i][j] = mulTables[i][antiLogTable[255 - logTable[j]]];
        }
    }
}

galois::gf galois::mul(galois::gf a, galois::gf b) {
    return mulTables[a][b];
}

galois::gf galois::div(galois::gf a, galois::gf b) {
    return divTables[a][b];
}

galois::gf **galois::cauchyMatrix(int m, int k) {
    if (m > 256) {
        printf("m should less than 256");
        return nullptr;
    }
    if (m < k) {
        printf("m should be equals with or greater than k");
        return nullptr;
    }
    auto **cauchyMatrix = reinterpret_cast<gf **>(malloc(sizeof(void *) * m));
    for (int i = 0; i < m; i++) {
        cauchyMatrix[i] = reinterpret_cast<gf *>(calloc(sizeof(gf), k));
    }
    for (int i = 0; i < k; i++) {
        cauchyMatrix[i][i] = 1;
    }
    for (int i = 0; i < m - k; i++) {
        for (int j = 0; j < k; j++) {
            cauchyMatrix[k + i][j] = div(1, (k + i) ^ (j));
        }
    }
#ifdef debug
    for (int i = 0; i < m; i ++) {
        for (int j = 0; j < k; j ++) {
            printf("%d ", (int)cauchyMatrix[i][j]);
        }
        printf("\n");
    }
#endif
