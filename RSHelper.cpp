...
#include "RSHelper.h"
//G(2^8) log2num
unsigned short RSHelper::num2field[256] = {
        ...
        1};
//G(2^8) num2log
unsigned short RSHelper::field2num[256] = {
        ...
        175};
int RSHelper::generatorPolynomial[256] = {};
int RSHelper::generatorPolynomialTemp[256] = {};
int RSHelper::currentRSCodeLength = -1;

/*
 * 生成reed-solomon纠错算法的生成多项式 保存在generatorPolynomial中
 * @param polynomialLength 表示所需生成的多项式的最高次数项次数
 */
void RSHelper::generateGeneratorPolynomial(unsigned short polynomialLength) {
    ...
    for (int i = 3; i <= polynomialLength; i++) {
        ...
        if (generatorPolynomialTemp[0] > 255) {
            generatorPolynomialTemp[0] %= 255;
        }
        for (int j = 1; j < i; j++) {
 ...
 */
void RSHelper::attachRSCode(int *originMessage, unsigned short messageLength,
                            unsigned short rsCodeLength) {
    thread_local int tempPolynomial[256];
    ...
    for (int i = messageLength + rsCodeLength; i >= rsCodeLength; i--) {
        if (tempPolynomial[i]) {
            ...
            for (int j = 0; j <= rsCodeLength; j++) {
                ...
                tempFactor = (tempFactor + addFactor) % 255;
 ...
 */
bool RSHelper::getOriginMessage(int *message, unsigned short messageLength,
                                unsigned short rsCodeLength) {
    thread_local int polynomialValue[256];
    thread_local int solveMatrix[129][129];
    int isCorrect = 0;
    for (int i = 0; i < rsCodeLength; i++) {
        int tempAns = 0;
    ...
    for (int i = 0; i < rsCodeLength; i++) {
        for (int j = 0; j < sumWrongPos; j++) {
            solveMatrix[i][j] = num2field[(wrongPos[j] * i) % 255];
        }
