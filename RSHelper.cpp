...
bool RSHelper::GetOriginMessageFromPackets(unsigned char** packets, int originalLength, int rsLength, size_t packetSize) {
    ...
    for (auto currentBatch = 0; currentBatch < batch; currentBatch ++) {
        auto copySize = std::min(packetSize - currentBatch * batchLength , batchLength);
        std::cout << currentBatch * batchLength << " " << copySize << std::endl;
        if (!getOriginMessage1(packets, originalLength, rsLength, currentBatch * batchLength,  copySize)) {
            return false;
        }
    }
   return true;
...
bool RSHelper::getOriginMessage1(unsigned char** packets, int originalLength, int rsLength, int offset, int  batchLength) {
    ...
    for (int i = 0; i < rsCodeLength; i++) {
        ...
        for (int j = rsCodeLength + messageLength - 1; j >= 0; j --) {
            auto word = packets[j / batchLength][j % batchLength];
            if (!word) {
                continue;
            }
    ...
    while (true) {
        for (int i = 0; i < maxWrongPos; i++) {
            memcpy(solveMatrix[i], polynomialValue + i, maxWrongPos);
        }
        // 开始将矩阵转换为行阶梯形式
        for (int i = 0; i < maxWrongPos; i++) {
            bool done = false;
    ...
    for (int i = 0; i < matrixRank; i++) {
        memcpy(solveMatrix[i], polynomialValue + i, matrixRank);
    }
    // 高斯消元法进行定位多项式系数求解
    for (int i = 0; i < matrixRank; i++) {
        if (!solveMatrix[i][i]) {
