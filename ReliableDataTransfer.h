...
static std::atomic_uint16_t currentSeq;
static std::atomic_uint16_t ackSeq;
static uint8_t **buffers;
static uint8_t **sendBuffers;
static std::once_flag t;
static std::atomic_uint16_t ack[windowSize];
...
void init() {
    ...
    sendBuffers = reinterpret_cast<uint8_t**>(malloc(sizeof(uint8_t **) * windowSize));
    for (int i = 0; i < windowSize; i ++) {
        ...
        if ((uint16_t)(curSec + 1) == seq) {
            for (uint16_t i = 0; i < windowSize; i ++) {
                ...
                if (finish[tempPos].load()) {
                    ...
                    //std::cout << "acked" << ackSeq.load() << std::endl;
